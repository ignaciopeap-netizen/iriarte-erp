...
async function boot(){
  ...
  if(send)send.onclick=async()=>{let e=document.getElementById("resetEmail").value.trim(),m=document.getElementById("resetMsg");if(!sb){m.textContent="Necesitas conexión a Supabase.";return}let r=await sb.auth.resetPasswordForEmail(e,{redirectTo:location.origin+location.pathname});m.textContent=r.error?r.error.message:"Enlace enviado."};
  if(save)save.onclick=async()=>{let a=document.getElementById("newPassword1").value,b=document.getElementById("newPassword2").value,m=document.getElementById("newPasswordMsg");if(a.length<8||a!==b){m.textContent="Las contraseñas no coinciden o son demasiado cortas.";return}let r=await sb.auth.updateUser({password:a});if(r.error){m.textContent=r.error.message;return}m.textContent="Contraseña actualizada.";history.replaceState(null,"",location.pathname);if(r.data?.user)await startForUser(r.data.user,false)};
  ...
  const showRecovery=()=>{document.getElementById("loginFields").style.display="none";document.getElementById("resetFields").style.display="none";document.getElementById("newPasswordFields").style.display="block";showLock(true)};
  const hashType=new URLSearchParams(location.hash.replace(/^#/,"")).get("type");
  sb.auth.onAuthStateChange(async(event,sessionNow)=>{if(event==="PASSWORD_RECOVERY"){showRecovery();return}if(event==="SIGNED_IN"&&sessionNow?.user&&hashType!=="recovery"&&getComputedStyle(document.getElementById("lockScreen")).display!=="none")await startForUser(sessionNow.user,false)});
