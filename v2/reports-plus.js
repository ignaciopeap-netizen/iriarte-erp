...
async function render(){
 ...
 <div class="card panel" style="margin-top:14px"><h3>Rentabilidad por proyecto</h3><p style="font-size:11px;color:var(--muted)">El margen directo solo descuenta compras y horas. Los gastos generales vinculados aparecen en columnas separadas para no alterar esa métrica.</p>${projects?.length?`<div class="table-wrap"><table class="table"><tr><th>Proyecto</th><th>Cliente</th><th>Facturado base</th><th>Cobrado</th><th>Compras</th><th>Horas</th><th>Coste horas</th><th>Margen directo</th><th>Margen %</th><th>Gastos vinculados</th><th>Resultado tras vinculados</th></tr>${projects.map(x=>{const pct=num(x.ingresos_facturados)?num(x.margen_directo)/num(x.ingresos_facturados)*100:0,oh=overheadFor(x.proyecto_id),after=num(x.margen_directo)-oh;return `<tr><td><b>${esc(x.proyecto||'')}</b></td><td>${esc(x.cliente||'—')}</td><td>${money(x.ingresos_facturados)}</td><td>${money(x.cobrado)}</td><td>${money(x.costes_compras)}</td><td>${num(x.horas).toLocaleString('es-ES',{maximumFractionDigits:1})} h</td><td>${money(x.coste_horas)}</td><td class="${num(x.margen_directo)<0?'negative':'positive'}"><b>${money(x.margen_directo)}</b></td><td>${pct.toLocaleString('es-ES',{maximumFractionDigits:1})}%</td><td>${money(oh)}</td><td class="${after<0?'negative':'positive'}"><b>${money(after)}</b></td></tr>`}).join('')}</table></div>`:'<div class="empty">Aún no hay proyectos para analizar.</div>'}</div>
