...
(function(){
  ...
  async function ensureProject(p){
    ...
    if(p.proyecto_id){
      const {data}=await client.from('proyectos').select('id').eq('id',p.proyecto_id).maybeSingle();
      if(data)return p.proyecto_id;
    }
    const payload={nombre:p.name||p.nombre||'Proyecto',cliente_id:p.cliente_id||null,codigo:p.ref||p.numero||null,direccion:p.address||null,estado:'activo',fecha_inicio:dateValue(p),expediente:p.expte||null,importe_contratado:budgetBase(p),descripcion:'Creado desde presupuesto '+(p.ref||p.numero||'')};
  ...
  async function convert(){
    ...
    try{const existed=!!p.proyecto_id;const pid=await ensureProject(p);window.APP.sel.project=pid;if(!existed)alert('Proyecto creado y vinculado al presupuesto.');location.hash='#proyectos';location.reload()}
  ...
  async function createInvoice(){
    ...
    try{
      ...
      const payload={numero:null,fecha:today(),cliente_id:p.cliente_id||null,proyecto_id:pid,presupuesto_id:p.id,concepto:p.name||p.nombre||'Presupuesto',estado:'borrador',irpf_pct:budgetIrpfPct(p),notas:'Factura creada desde presupuesto. Editable independientemente del presupuesto.'};
