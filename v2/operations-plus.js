...
(function(){
  ...
  function projectId(){
    const S=window.APP;if(!S)return'';
    if(Object.prototype.hasOwnProperty.call(S.sel||{},'project'))return S.sel.project||'';
  ...
  function renderObra(){
    ...
    ${pid?`<div class="notice"><b>${esc(pName(pid))}</b> · seguimiento de obra, tareas e incidencias del mismo proyecto.</div>`:'<div class="notice">Mostrando la actividad de obra de todos los proyectos.</div>'}
  ...
  async function completeTask(id){const {error}=await db.from('obra_tareas').update({estado:'completada',fecha_completada:new Date().toISOString().slice(0,10)}).eq('id',id);if(error)alert(error.message);else if(window.reloadIriarte)await window.reloadIriarte();else location.reload()}
  async function resolveIncident(id){const {error}=await db.from('obra_incidencias').update({estado:'resuelta',fecha_resolucion:new Date().toISOString().slice(0,10)}).eq('id',id);if(error)alert(error.message);else if(window.reloadIriarte)await window.reloadIriarte();else location.reload()}
